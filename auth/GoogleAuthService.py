...
GOOGLE_CLIENT_SECRET = config("GOOGLE_CLIENT_SECRET", cast=Secret, default="YOUR_GOOGLE_CLIENT_SECRET")
...
class GoogleAuthService(AuthService):
    ...
    def authenticate(self, token: str) -> User:
        ...
        return User(id=user_id, email=f"{user_id}@google.com", provider="google", display_name="Mock User")
    ...
    async def auth_callback(self, request: Request) -> Response:
        ...
        if not user:
            return HTMLResponse(content="<p>Authentication failed.</p>", status_code=401)
        ...
        response = HTMLResponse(content=f"<p>Authenticated as {user.display_name}. <a href='/api/v1/items/1'>Test API</a> | <a href='/auth/logout?provider=google'>Logout</a></p>")
